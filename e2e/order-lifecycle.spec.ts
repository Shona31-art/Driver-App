...
async function login(page: Page, email: string) {
  ...
  await page.waitForURL(/\/(admin|driver)\/dashboard/, { timeout: 15_000 });
}
...
test("full order lifecycle: create -> assign -> confirm -> load -> deliver -> expense -> review -> complete", async ({
  ...
  await test.step("Open the new order from the list", async () => {
    ...
    await page.waitForURL(/\/admin\/orders\/[0-9a-f-]+$/, { timeout: 15_000 });
    orderUrl = page.url();
  ...
  await test.step("Driver confirms the assignment", async () => {
    ...
    await expect(page.getByText(/Assignment confirmed/)).toBeVisible({ timeout: 15_000 });
  });
  ...
  await test.step("Driver marks the load as Loaded", async () => {
    ...
    await expect(page.getByText(/Marked as loaded/)).toBeVisible({ timeout: 15_000 });
  });
  ...
  await test.step("Driver marks the load as Delivered", async () => {
    ...
    await expect(page.getByText(/Marked as delivered/)).toBeVisible({ timeout: 15_000 });
  });
  ...
  await test.step("Driver logs an expense linked to this order", async () => {
    ...
    await page.waitForURL(/\/driver\/expenses\?orderId=/, { timeout: 15_000 });
    ...
    await expect(page.getByText(/Expense submitted/)).toBeVisible({ timeout: 15_000 });
  });
  ...
  await test.step("Approve the driver's expense", async () => {
    ...
    await expect(page.getByText(/Expense approved/)).toBeVisible({ timeout: 15_000 });
  });
  ...
  await test.step("Mark the order as completed", async () => {
    ...
    await expect(page.getByText(/Order marked as completed/)).toBeVisible({ timeout: 15_000 });
  });
