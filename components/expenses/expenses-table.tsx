      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-canvas/60 hover:bg-canvas/60">
            ...
            <TableHead className="h-11 w-52 px-4" />
          </TableRow>
        </TableHeader>
        <TableBody>
          {expenses.map((expense) => (
            <TableRow key={expense.id}>
              <TableCell className="px-4 py-3.5 text-slate">{expense.driver_name}</TableCell>
              ...
              <TableCell className="px-4 py-3.5">
                {expense.status === "pending" && <ExpenseReviewActions expenseId={expense.id} />}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
